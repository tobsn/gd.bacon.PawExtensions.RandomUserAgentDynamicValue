var RandomUserAgentDynamicValue = function(){
	this.title = function(){return 'Random User Agent'};
	this.evaluate = function(){
		var max=0,rcount=0,type='';
		for(var k in this.freq){max+=this.freq[k]}
		var roll=Math.floor(Math.random()*(max+1));
		for(var k in this.freq){if((roll<=(rcount+=this.freq[k]))&&(!type)){type=k}}
		return this.agents[type].sort(function(){return 0.5-Math.random()})[0];
	}
	this.freq = {
		'IE':11.8,
		'Firefox':28.2,
...
RandomUserAgentDynamicValue.identifier = 'gd.bacon.PawExtensions.RandomUserAgentDynamicValue';
RandomUserAgentDynamicValue.title = 'Random User Agent Dynamic Value';
RandomUserAgentDynamicValue.inputs = [];
registerDynamicValueClass(RandomUserAgentDynamicValue);